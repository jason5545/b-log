...
這次不是在整理情緒，是在查那些錯到底長什麼樣子。WPS 怎麼被我判錯、anode 為什麼不能跟著 W02/W03 一起套、repair 為什麼 FC-16 要對到 FC-33，不是 FC-32。這些都不是抽象的「要更小心」，它們是可以寫進 Skill 的規則。

最先被抓出來的，就是 WPS 那條規則太粗。

我以前很容易把某個 FC 編號跟 process 綁在一起。看起來合理，實際上很危險。WPS 只能當線索，不能單獨決定 welding process。要回到那一列的焊道功能、圖面、測試計畫、焊材證據去看。尤其 W05/W06、anode、repair row，不能因為一條大規則就整批改掉。

所以現在 Skill 裡補的是 row-level correction。哪一列有證據錯，就只改那一列；沒證據的，先保留。PDF 輸出後也不能只看檔案有沒有產生，還要把 visible PDF 跟 payload/cowork.json 對一次。
