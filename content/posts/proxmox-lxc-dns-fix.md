## 在 Proxmox 上被 DNS 搞瘋的一天：我怎麼幫所有 LXC 容器「定住」 `/etc/resolv.conf`

最近我在自己的 Proxmox 節點上，被一個看似小事但超煩的問題折騰了好一陣子：**所有 LXC 容器的 DNS 設定會自己跑掉**。這篇文章記錄我從「一直手動修」到「完全自動化、永久穩定」的過程，也順便整理在 Proxmox LXC 裡，DNS 到底怎麼被決定、又是誰在偷偷改它。
...
對我來說，做完這三件事之後，DNS 就從「每天惹事的鬼」變成「完全忘記它存在」。
