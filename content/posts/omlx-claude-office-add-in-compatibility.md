...
## 更新：middleware 接上 Python code execution

最初的做法是把 `code_execution_20250825` 直接 strip 掉——oMLX 不認識沒有 `input_schema` 的 Anthropic built-in tools，送進去會 422，所以先移除再說。

後來查了 Anthropic 文件才確認：`web_search_20250305` 和 `code_execution_20250825` 是 Anthropic **server-side tools**，不是一般 client-side custom tools。在官方 Claude API 裡，執行這些工具是 server 的責任，response 裡可能出現的是：

- `server_tool_use`
- `web_search_tool_result`
- `bash_code_execution_tool_result`
- `text_editor_code_execution_tool_result`
- 最後才是 final text answer

也就是說，光補 `input_schema` 是不夠的。oMLX 只負責推理，不會自己去執行程式碼。要在本機支援這些工具，middleware 就得自己變成一個簡化版 tool orchestrator，把完整的 tool execution loop 接起來。

這次先接了 Python-only code execution 的 MVP。

### 轉換流程

Office add-in 送來的是 Anthropic server-side format：

```json
{"type": "code_execution_20250825", "name": "code_execution"}
```

middleware 把它轉成 oMLX 可以理解的 custom tool：
...
1. Office add-in 發 request，裡面包含 `code_execution_20250825`
2. middleware 把 model 從 `claude-sonnet-4-5-20250929` rewrite 成 `Qwen3.6-35B-A3B-MLX-8bit`
3. middleware 把 `code_execution_20250825` 轉成 custom tool `code_execution`
4. oMLX / Qwen 回 `tool_use`
5. middleware 取出 tool input 裡的 Python code
6. middleware 在本機 sandbox 裡執行 Python
7. middleware 把 stdout / stderr / return code 包成 `tool_result`
8. middleware 再呼叫 oMLX 一次
9. final answer 回給 Office add-in
10. response 裡的 model 改回 `claude-sonnet-4-5-20250929`

oMLX 本身不理解 Anthropic server-side tools；這一層是 middleware 做的 compatibility translation 加上 local Python execution loop。

### 安全限制

這不是把模型產生的程式碼拿去本機無限制執行。目前的最小 sandbox：

- Python only，不提供任意 shell 存取
- 每次跑在 temporary directory
- 使用 macOS `sandbox-exec`
- 不允許 network
- 不允許讀取使用者 home directory（`~/.ssh`、`~/.omlx/settings.json` 等）
- 不允許寫入 temporary directory 以外的位置
- timeout 20 秒
- stdout / stderr 有長度上限

### 測試結果

用 gateway smoke test 驗證過：

- `code_execution_20250825` 可被轉成 local `code_execution`
- Qwen 會產生 `tool_use`
- middleware 能執行 Python 並回送 `tool_result`
- final answer 能正常回給 Office-compatible client
- streaming request 也會被 middleware 包成簡化 SSE 回應
- model rewrite 仍正常，oMLX 實際只載入 `Qwen3.6-35B-A3B-MLX-8bit`

### 目前限制

這只是 MVP，不是完整的 Anthropic Code Execution clone。還沒有：

- `bash_code_execution`
- `text_editor_code_execution`
- Files API upload / download
- container reuse
- 產生圖檔後讓 Office 下載 artifact
- 官方 server-side response block 的完整模擬

但對 Excel agent 常見的需求——用 Python 做計算、統計、表格整理、資料轉換——已經夠先用。

`web_search_20250305` 目前仍然先 strip 掉，還沒有接搜尋 API。下一步可以用類似方式接成 middleware search augmentation，但這次更新先專注在 Python code execution。

---

## 更新：從「能回文字」到「能操作 Excel」

接上 code execution 之後，以為差不多了。結果又踩了一整晚。

問題的起點是 Excel 有時候等很久，然後顯示 Rate Limit Exceeded。

看 gateway log，不是真的 rate limit。是 Office add-in 自己在 retry。Office 大概等 15 秒沒看到 meaningful response 就會重送一次。本機 Qwen 35B 加上 Python tool loop，一個 request 跑超過 15 秒很正常。

第一版的應對是加 single-flight：同一時間只跑一個 code execution。結果 Office retry 進來撞到 single-flight，gateway 直接回 429。Excel 看到 429 就顯示 Rate Limit Exceeded，使用者不知道發生什麼事。
...
Office 的 JS SDK 根本不把 SSE comment 當成有效進度，該 retry 還是 retry。

改成真的送 Anthropic-style ping，每 2 秒一次：
...
這讓 Office 在等模型推理和 Python tool loop 的時候，比較不會誤判成 timeout。

### 再修 retry 撞 429

光有 heartbeat 還不夠。Office 在連線重建的瞬間還是會重送，single-flight 還是讓 retry 吃到 429。

後來改成用 request body hash 做 coalescing：

- 第一個 request 正常跑
- Office retry 帶著一模一樣的 body 進來，不回 429
- gateway 開一條新的 SSE 連線，掛著等原本那個 request 跑完
- 完成後把同一份結果 stream 回給 retry 連線

這樣本機 35B 不會被多重 generation 打爆，Office 也不會看到 429。

### 然後才發現真正的問題

以為這樣就修完了。結果有一次 Excel 顯示「資料已計算完成，現在寫入 Excel。」但什麼都沒寫進去。

看 gateway log，模型最後其實有回 `tool_use`，stop_reason 也是 `tool_use`。但 gateway 的 SSE serializer 只把 text block 串成文字 stream 出去，`tool_use` block 完全沒送給 add-in。

Excel 只看到那段文字，沒有收到 `set_cell_range` 或 `execute_office_js`，根本不知道自己應該寫入什麼。那段「資料已計算完成」是模型在 tool_use 之前自己生成的說明文字，不是結果。

### 把 SSE serializer 補完整

問題很清楚了，就是 SSE serializer 沒有處理 tool_use 的 streaming format。補起來：

- text block 用 `text_delta`
- tool_use block 用 `content_block_start`，帶 `id`、`name`、`input: {}`
- input 的內容用 `input_json_delta` 送 `partial_json`
- 最後維持 `message_delta` 和 `message_stop`

補完後再測，Excel 成功建立銷售資料表、寫入公式和分析摘要。整條鏈才真的通了。

回頭想，這個過程有點像洋蔥：以為是 timeout → 其實是 Office retry 太 aggressive → heartbeat comment 根本沒用 → 換成 ping 之後 retry 減少了，但 single-flight 還是讓它看到 429 → 加了 coalescing 之後 retry 不再是問題 → 然後才發現原來 SSE serializer 從一開始就沒有把 tool_use 送出去。
