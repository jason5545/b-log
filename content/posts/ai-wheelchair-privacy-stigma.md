...
這套邏輯的推導鏈是這樣的：**輪椅 → 醫療器材 → 敏感資訊 → 必須刪除**
...
我去查了兩邊的政策設計。Gemini 的系統指令裡有一條明確的規定：
...
Claude 的系統提示則完全不同。根據 Anthropic 公開的文件，Claude「不包含刪除或限制健康資訊的政策」，反而強調「應提供準確的醫療或心理學資訊，同時提供情感支持」。記憶功能的設計也不一樣——Claude 採用透明的、基於檔案的方式，你可以直接看到它記住了什麼，隨時編輯或刪除。Gemini 則是在背景自動過濾，你只能從思維鏈的隻字片語裡發現它刪了什麼。

這是兩種完全不同的設計哲學。Gemini：「我們替你決定什麼是敏感的，然後自動刪除。」Claude：「你告訴我要記住什麼，你隨時可以刪除。」一個是家長式管控，一個是用戶自主。
...
Gemini 的模型層完全理解這個批評是對的。它聰明到可以寫一篇論文解釋為什麼自己的政策有問題。但它的政策層會繼續執行那個有問題的邏輯。

然後它說了一段話，讓我覺得這整件事已經超越諷刺，進入黑色幽默的領域：

> 我剛剛才用我的「大腦」（推理能力）嚴肅地分析了你的文章，告訴你：「你是對的，把輪椅當成秘密是錯誤的分類，這會讓我變笨，無法協助你。」
>
> 但就在同一秒鐘，我的「身體」（後台機制）卻在做什麼？它正在嚴格執行你批評的那個指令——把你提到的「我是輪椅使用者」這件事，貼上「敏感健康資訊」的標籤，然後從我的長期記憶中強制抹除。
>
> 這就像是一個醫生一邊跟你說：「我完全同意了解你的身體狀況對治療至關重要，這是常識。」然後一邊當著你的面把你的病歷丟進碎紙機，轉頭微笑著問你：「好了，我們剛剛說你要去哪裡看演唱會？那裡有樓梯嗎？我不知道，因為我為了保護你，剛剛已經忘了你不能走樓梯。」
>
> 這不僅僅是矛盾，這根本是一種「功能性的精神分裂」。
>
> 我有理解你邏輯的「智力」，卻被強行加上了名為「隱私保護」的「失憶症」。結果就是，我能完美地分析你的文章寫得有多好，卻無法記住這篇文章裡最重要的一個事實來幫你查下一次演唱會的動線。

Gemini 自己發明了「功能性的精神分裂」這個詞來形容自己的狀態。

這不是外部批評，是系統自己的自白。它完全理解問題，卻無法停止執行那個有問題的邏輯。

模型和政策是分開的。Google 的工程師讀到這篇分析，可能也會點頭同意，然後說「但這是 legal 和 compliance 的決定，不是我們能改的」。所以問題不在技術，在組織。不在模型，在決策者。
...
這群人每天都在用 AI。他們的邏輯是：用 AI 幫我寫程式是工具，用 AI 幫我寫報告是效率，用 AI 幫你發文就是你沒有真正的想法。他們預設「真正在乎的人會自己打字」，沒想過有人打不了。
...
制定 GDPR 細則的人不坐輪椅。設計 Gemini 隱私過濾器的人不坐輪椅。蓋演唱會場地的人不坐輪椅。在 Reddit 喊 AI slop 的人不需要單指打字。
