...
2022 年，蘋果在 iPadOS 16 推出 DriverKit，聲稱能讓開發者為音訊、USB、PCI 設備建立驅動程式。**兩年多過去了，實際狀況是什麼？**

根據技術文件和製造商說明：
- iOS/iPadOS 設備（截至 17.3 版本）仍主要要求設備必須是 class-compliant
- **目前沒有任何音訊介面有自訂 iOS 驅動程式**
- PreSonus 等專業音訊設備製造商明確表示：「Apple iOS/iPadOS 設備不支援安裝自訂設備驅動程式」

**結果：**
- 專業音訊介面：只能用 class-compliant 的，高階設備不支援
- 外接顯示卡：完全不可能
- 專業印表機：很多不支援
- 其他專業設備：運氣問題
...
**技術細節：**
三星在 bootloader 配置中加入新代碼行：
```
androidboot.other.locked=1
```
這行代碼會告訴系統隱藏「OEM 解鎖」開關。

**可能原因：**
EU 法規 2014/53/EU 在 2025 年 8 月 1 日生效，要求手機製造商保護設備免受未經授權的軟體影響。
...
**嚴格限制：**
- **從 3 台降到 1 台**：每年只能解鎖 1 台設備（原本是 3 台）
...
- **單一設備綁定**：所有步驟（測驗、註冊、申請、綁定、解鎖）必須在同一台設備完成
...
**對開發者的打擊：**
許多開發者需要多台設備進行測試和開發，一年一台的限制嚴重影響開發工作。這也可能導致：
- 客製化 ROM 開發延遲
- 對較舊或較不受歡迎設備的支援減少
- POCO F1、Redmi K20、Redmi Note 等經典設備的社群支援逐漸消失
...
- **設備限制**：電信合約機、政府/企業版本完全不允許解鎖
- **目前影響範圍**：僅限中國銷售的 ColorOS 16 設備

**官方說法：**
> 「為確保設備和資料安全並增強系統穩定性，我們將調整 ColorOS 16 的 bootloader 解鎖方法。」

**社群反應：**
- 許多使用者表示「當初選擇 OnePlus 就是因為可以解鎖 bootloader」
- 擔心未來會擴展到全球 OxygenOS 設備
...
F-Droid 表示無法代表開源貢獻者接管 app 身份，這意味著：
...
- 使用者需與開發者分享設備識別碼
- 開發者將其輸入 Google 控制台
- **Google 控制和限制未驗證開發者的 app 可安裝到多少設備**
...
**短期策略：**
1. 如果需要自由度：
   - 盡快購買 One UI 8 之前的三星設備
   - 考慮還能自由解鎖的小米舊設備（2024 年底前的型號）
   - 選擇 ColorOS 15 的一加設備
2. 避免更新到會移除自由度的系統版本
3. 備份重要的開發環境和工具
...
3. 關注開源替代方案的發展（如 postmarketOS、Linux 平板）
...
- F-Droid（開源 Android app 商店）
- Windows 11 Insider 更新
- iPadOS 更新說明

**記住：你花錢買的設備，應該是你真正擁有的。**
