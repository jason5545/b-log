...
**結果**:
- ❌ 某些程式即使收到 CP950 的 Alt 鍵加上數字鍵台序列，仍會強制轉換為 CP1252，導致中文字元無法正確顯示
- ❌ 在 RDP 環境中完全無效
- ❌ 只支援 Big5 字元(約 13,000 個字)

**結論**:更改鍵盤的實作邏輯，優先使用方案二（Alt+X Unicode 模式），方案一（Big5 Alt 碼）作為備援，兩者互補以提升中文輸入的相容性和可靠性。
...
| 方法 | 單字時間 | 「哈囉」時間 | 相對速度 |
|------|---------|------------|---------|
| Big5 Alt 碼 | ~600ms | ~1200ms | ❌ 最慢(24x) |
