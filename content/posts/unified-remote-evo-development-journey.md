...
這個專案源於我自己的特殊需求：由於我僅右手食指可活\，需要一個能透過 Android 平板遠端控制 PC 的解決方案。原本的 Unified Remote 和 EmulStick 雖然都是不錯的工\，但**兩者都已停止維護和更新**,且缺乏針對像我這樣有身體限制使用者的最佳化。

### 為什麼這些軟體停止更新很重要?

- **Unified Remote**:官方已停止維\，Android 客戶端不再更\，協定規格凍結
- **EmulStick**:硬體接收器韌體已穩\，不再改版
...
**這正是我開發新客戶端的契機**：既然官方不再更\，我可以自由研究協定、客製化功\，為這些停更的工具注入新生命。

### 產品優勢整合

這兩個產品各有優勢:
- **Unified Remote**:功能豐\，可透過網路(Tailscale)遠端控制
- **EmulStick**:即插即\，無需伺服器軟\，但需要藍牙接收器(一個小型USB裝\，可將Android裝置變成PC的鍵盤/滑鼠/遊戲手把)

**EmulStick 接收器簡介**:
EmulStick 是一款創新的藍牙接收\，外觀類似小型USB隨身碟。插入PC\，會建立一個藍牙HID(人機介面裝置)連\，讓Android平板或手機能夠直接模擬成PC的鍵盤、滑鼠和遊戲手把。與傳統的軟體解決方案不\，EmulStick透過硬體層面的模\，確保了極低的延遲和最高的相容\，幾乎所有PC遊戲和應用程式都能完美支援。

於是我決定打造一個整合方\，讓自己能根據情境選擇最適合的連線模式。

## 第一個挑戰:研究 BLE HID 協定

**背景**：由於 EmulStick 官方已停止更\，無法從官方文件取得協定規格。我必須透過研究原廠 APP 的實作來了解 BLE 通訊協定。
...
這個矛盾讓我陷入困境。經過深入研究原廠 APP 的程式\，我發現了問題所在:
...
但實際上 EmulStick 的廣播封包可能不包含 Service UUID,導致 Phase 1 完全無法命\，而 Phase 切換過程中又可能遺失裝置。
...
結果立刻成功!這個經驗讓我學到:**不要過度最佳\，先求能\，再求最佳化**。

### 身份驗證:AES 加密挑戰

成功掃描到裝置\，下一個挑戰是身份驗證。EmulStick 使用 AES 加密來防止未授權存取:
...
雖然這個機制看似複\，但所有關鍵資訊(明文密碼、加密算法)都在原廠 APP \，因此只要完全複製邏輯就能通過驗證。

### 突破時刻:找到正確的 HID Report 格式

身份驗證完成\，連線成\，但所有滑鼠/鍵盤操作都無反應!

經過深入研\，我發現 EmulStick 有兩個版本:
...
HID 鍵盤只能傳送 ASCII 字\，那中文怎麼辦?我研究了三種方案:
...
研究 EmulStick 協定\，我發現它支援切換為 Xbox 360 控制器模式!
...
初期遇到一個 bug:調整左扳機\，右扳機被重置為 0;調整右扳機\，左扳機被重置為 0。

**原因**:每個扳機組件內部都有獨立的狀\，互相覆蓋。
...
在完成 Android 版本\，我曾研究是否能將此專案移植到 iOS 平台。然\，我遇到了 **iOS 平台的根本性限制**：
...
若要在 iOS 上實作類似 Unified Remote 的藍牙功\，需要：
...
1. **完全模仿原廠架構**:不要過度最佳\，先求能動
2. **詳細日誌記錄**:診斷 APP 的詳細日誌是成功關鍵
3. **隔離測試**:建立獨立的診斷 APP,避免複雜邏輯干擾

### 避免的陷阱

1. **過度設計**:兩段式掃描聽起來很聰\，但實際上增加複雜度
2. **假設問題所在**:一開始以為是 UUID 問\，實際是掃描流程問題
...
這個專案從我個人的簡單需求出\，逐步演化為一個功能完整的多模式遙控解決方案。開發過程中遇到的每個技術挑\，都成為學習和成長的機會。

### 為停更軟體注入新生命

當 Unified Remote 和 EmulStick 官方停止維護\，許多使用者面臨著「好用的工具逐漸不相容新系統」的困境。這個專案證明了:

- ✅ **開放原始碼精神的價值**:即使官方停\，社群仍能延續產品生命
- ✅ **協定研究的可行性**:透過技術分\，可以實作完全相容的客戶端
- ✅ **客製化的自由**:不受原廠限\，可以針對特殊需求最佳化
- ✅ **現代化改造**:使用最新技術棧（Kotlin、Jetpack Compose）重新實作

最重要的\，這個專案實現了它的初衷:為我自己（以及其他有身體限制的使用者）提供一個靈活、高效的 PC 控制方案。
...
**核心理念**:
- 📦 **延續產品生命**:當官方停止更\，社群接手維護
- 🔓 **協定開放**:研究並實作相容協\，打破原廠限制
- ♿ **無障礙最佳化**:針對身體限制使用者進行深度最佳化
- 🆕 **現代化技術**:使用最新工具重新打\，確保長期可維護
