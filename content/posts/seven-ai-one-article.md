...
智譜的 GLM-5：7,450 億參數，完全用華為晶片訓練，開源 MIT 授權，API 定價是西方模型的六分之一。我也拿它測了那篇文章——思考鏈裡的理解深度不輸任何一家，只是輸出端還沒完全跟上。Kimi K2.5：1 兆參數，Agent Swarm 技術可以同時協調 100 個子代理平行工作，成本比頂尖閉源模型低 76%。MiniMax 的 M2.5 同週更新（雖然繁體中文的基本功還沒到位）。DeepSeek 的 V4 月底要來。

這些模型全部開源或接近開源，在 coding 和 agentic benchmark 上已經跟閉源模型平起平坐。Stargate 的「算力護城河」假設，在開源模型能用更少算力做到差不多事情的世界裡，就是用沙子堆的。
