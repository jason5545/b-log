...
這篇寫完之後，我用同一顆模型（Jundot oQ 混合精度版）做了五輪對照實驗，對手是 GLM-5.3 Flash。題目從六個檔案的 TypeScript 改到 Kotlin 佇列服務，每輪有隱藏測試。全部用 low 跑。

結果跟上面那個三十八分鐘的 session 完全不像。五輪都是自己做完、跑完驗收、commit、停下，最長十五分鐘。四輪產出跟參考解逐 byte 相同。有一輪我什麼位置都不給，只給三個 bug 的症狀和驗收條件，它三個根因都找對，設計題也自己拍板，二十一條隱藏測試全過，跟 GLM 一樣。

差別只有三件事。第一，範圍在開場就定死，不是做到一半才收斂。第二，每個任務開新 session，context 從來沒超過一份 prompt 加一個小專案。第三，prompt 裡有驗收清單：跑哪幾條指令、輸出應該長什麼樣。那份清單就是「證據已經足夠」的定義。

所以上面說它缺的是「做決定然後交出來」，要修正成：它缺的是一個寫在 prompt 裡的完成條件。給了，它就會交。這件事不用等 Pi 做 completion check，自己寫就有。

推理等級也不是重點。我後來請 Codex 分析 low 和 xhigh 的差別，結果只是 system prompt 多一句「想少一點」或「想詳細一點」，medium 連這句都沒有。上面那個 session 用 xhigh 跑飛，這五輪用 low 全過，差在題目的形狀，不在那一句。
