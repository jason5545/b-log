...
Opus 5.5 的輸出分三種區塊：thinking、text、tool_use。Opus 5 在工具呼叫之間寫的說明是 text，就是它自己寫的字。Opus 5.5 改了，工具呼叫之間的說明改成 thinking 區塊回傳。

thinking 區塊裡本來就不是原始的推理，拿到的都是摘要。文件寫明，摘要是另一個模型處理的，負責思考的那個模型也看不到摘要。

所以我在畫面中間讀到的，已經是另一個模型改寫過的版本。

我跟它查這件事的那個 session 就有例子。它說它原本寫的進度說明是「只看開頭 450 字，網太窄了」，紀錄裡存的是「且僅檢查前 450 字，覆蓋範圍太窄」。意思一樣，字全換了。
...
比較確定的是管道換了。以前中間的進度說明是 Opus 自己寫的，現在也要先經過另一個模型。
...
而且 Opus 5.5 真的省很多。Max 方案裡 Fable 最多只能用掉每週額度的一半，換成 Opus 5.5，整份訂閱都能用上。就算跟 Opus 5 比，API 定價也只要八成。
