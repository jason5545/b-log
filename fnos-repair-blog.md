...
我一開始逐個加入例外，後來乾脆設定了一條規則：**root 用戶的 HTTP/HTTPS 出站自動放行**。這樣 liveupdate 程序就能自由連接更新伺服器了。
...
這是最離譜的發現。更新腳本持續失敗，錯誤碼 127（命令未找到）。經過排查：
...
| 問題 | 風險等級 | 說明 |
|------|----------|------|
| Docker Manager 漏洞 | **高** | 這是駭客的入侵點，允許遠端執行代碼 |
| IPv6 預設放行 | **高** | `meta nfproto ipv6 accept` 繞過所有 IPv4 限制 |
| 服務以 root 運行 | **中高** | `trim_app_center`、`trim_license` 等都以 root 運行，一旦被攻破就是完全控制 |
